...
from src.backend.models import Planet, Fleet
...
class TestShipyardIntegration:
    ...
    def test_ship_building_workflow(self, client, sample_user, sample_planet):
        """Test complete ship building workflow"""
        headers = make_auth_headers(sample_user.id)

        # Set up planet with plenty of resources
        sample_planet.metal = 100000
        sample_planet.crystal = 50000
        sample_planet.deuterium = 25000
        ...
        assert data['planet_resources']['crystal'] == 50000 - expected_crystal_cost
        assert data['planet_resources']['deuterium'] == 25000 - expected_deuterium_cost

        # Verify fleet was created/updated
        assert data['fleet']['colony_ship'] == 3

    def test_ship_building_creates_fleet(self, client, sample_user, sample_planet):
    ...
    def test_ship_building_updates_existing_fleet(self, client, sample_user, sample_planet, sample_fleet):
...
class TestShipyardEdgeCases:
    ...
    def test_build_ship_max_quantity(self, client, sample_user, sample_planet):
        """Test building maximum quantity of ships"""
        headers = make_auth_headers(sample_user.id)

        # Set planet with massive resources
        sample_planet.metal = 10000000
        sample_planet.crystal = 5000000
        sample_planet.deuterium = 2500000
    ...
    def test_build_ship_exact_resources(self, client, sample_user, sample_planet):

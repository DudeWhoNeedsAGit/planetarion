...
from src.backend.services.tick import (
...
class TestTickService:
    ...
    def test_get_next_tick_number_no_previous(self, db_session):
        ...
        with patch('src.backend.services.tick.TickLog') as mock_ticklog:
    ...
    @patch('src.backend.services.tick.current_app')
    def test_run_tick(self, mock_app, db_session, sample_planet):
        ...
        with patch('src.backend.services.tick.get_next_tick_number', return_value=1), \
             patch('src.backend.services.tick.process_resource_generation', return_value=[]), \
             patch('src.backend.services.tick.process_fleet_movements', return_value=[]), \
             patch('src.backend.services.tick.log_tick'):

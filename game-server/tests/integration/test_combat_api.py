...
def test_get_combat_reports_with_data(client, db_session):
    ...
    response = client.get('/api/combat/reports', headers={'Authorization': f'Bearer {token}'})
...
def test_get_combat_report_detail(client, db_session):
    ...
    response = client.get(
        f'/api/combat/reports/{report.id}',
...
def test_get_debris_fields(client, db_session):
    ...
    response = client.get('/api/combat/debris', headers={'Authorization': f'Bearer {token}'})
...
def test_get_combat_statistics(client, db_session):
    ...
    planet = Planet(name='Stats Planet', x=100, y=200, z=300, user_id=user.id)
    db_session.add(planet)
    db_session.commit()
    ...
    response = client.get('/api/combat/statistics', headers={'Authorization': f'Bearer {token}'})
...
def test_combat_reports_pagination(client, db_session):
    ...
    response = client.get(
        '/api/combat/reports?limit=2&offset=1',
        headers={'Authorization': f'Bearer {token}'},
    )
